...
$dos = array('display', 'post','del', 'add', 'group','strategy', 'credit_record', 'credit_stat','download','up_excel','show_list');
...
if($do == 'display') {
    ...
    $sql = "select id,name from " . tablename ( 'ms_strategy' );
...
if($do == 'post') {
    ...
    $sql = "select id,info from " . tablename ( 'ms_strategy' );
    $strategy = pdo_fetchall($sql);//策略列表
...
if($do == 'show_list') {
    ...
    if($_W['isajax']) {
        $uid = intval($_GPC['uid']);
        $data = pdo_fetchall("SELECT b.name,c.info FROM " . tablename('xxx_user_strategy_table')." a 
                LEFT JOIN " . tablename('ms_strategy')." b on a.strategy_id=b.id 
                LEFT JOIN " . tablename('xsy_resource_file')." c on a.file_id=c.id 
                where a.user_id=".$uid);
